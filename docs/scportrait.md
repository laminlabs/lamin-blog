...
The technology that was used to generate the data we work with is 10x Genomics’s _Xenium_. Xenium enables the acquisition of two data modalities on the single cell level:
...
1. Loading Xenium data as a [SpatialData](https://spatialdata.scverse.org/en/latest/index.html) object that can be curated in lamin.ai
...
After loading the Xenium dataset into a SpatialData object we can inspect the tissue sample that was profiled. Figure 1 shows an H&E-stained overview of the tissue section.
